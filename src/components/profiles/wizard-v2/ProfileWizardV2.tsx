...
import { extractNrcCompatibility, type NrcCompatibilityData } from "../../../utils/doofie-compatibility";
...
export function ProfileWizardV2({ onClose, onSave, defaultGroup }: ProfileWizardV2Props) {
  ...
  useEffect(() => {
    const loadNrcCompatibility = async () => {
      try {
        const packsConfig = await invoke<NoriskModpacksConfig>("get_doofie_packs_resolved");
  ...
  const handleStep3Create = async (profileData: {
    ...
    const createParams: CreateProfileParams = {
      ...
      selected_doofie_pack_id: profileData.selectedNoriskPackId || undefined,
