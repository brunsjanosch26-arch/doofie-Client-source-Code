...
import { NoriskModEntryDefinition, NoriskModpacksConfig } from "../../../types/doofiePacks";
...
export function ProfileWizardV2Step3({
    ...
    const [doofiePacks, setNoriskPacks] = useState<Record<string, NoriskPack>>({});
    ...
    // Load Doofie packs on component mount
    useEffect(() => {
        const loadNoriskPacks = async () => {
            try {
                ...
                // Auto-select "doofie-prod" if available
                if (packsData.packs["doofie-prod"]) {
                    setSelectedNoriskPackId("doofie-prod");
                }
            } catch (err) {
                console.error("Failed to load Doofie packs:", err);
            } finally {
    ...
    const doofiePackOptions = Object.entries(doofiePacks)
        .filter(([packId]) => {
            ...
            return packId === "doofie-prod" || packId === "doofie-bughunter" || packId === "";
        })
        .map(([packId, packDef]) => ({
            value: packId,
    ...
    useEffect(() => {
        const checkPackCompatibility = async () => {
            ...
            try {
                ...
                // Check if the selected pack has Doofie Client mods for this version/loader
                ...
                // Check if any Doofie Client mod exists and is compatible with the selected version/loader
                const hasCompatibleDoofieClient = mods.some((mod: NoriskModEntryDefinition) => {
                    // Check if this is a Doofie Client mod
                    if (mod.id === "doofieclient-client" || mod.id === "nrc-client") {
                ...
                console.log("Has compatible Doofie Client:", hasCompatibleDoofieClient);

                if (!hasCompatibleDoofieClient) {
                    setShowYellowWarning(true);
                }
            } catch (err) {
                ...
                <div className="space-y-3">
                    ...
                    {showAdvancedSettings && (
                        <div className="space-y-4 p-4 bg-white/5 border border-white/10 rounded-lg">
                            {/* Doofie Pack Selection */}
                            <div className="space-y-3">
                                <label className="block text-base font-minecraft-ten text-white/50">
                                    {t('profiles.wizard.doofieClientPack')}
                                </label>
                                <p className="text-sm text-white/60 font-minecraft-ten">
                                    {t('profiles.wizard.doofiePackDescription')}
                                </p>
                                {loadingPacks ? (
                                    ...
                                    <>
                                        ...
                                        {showYellowWarning ? (
                                            ...
                                            selectedNoriskPackId && doofiePacks[selectedNoriskPackId] && (
                                                <div className="text-center">
                                                    <p className="text-sm text-white/70 font-minecraft-ten">
                                                        {doofiePacks[selectedNoriskPackId].description}
                                                    </p>
                                                </div>
                                            )
                                        )}
