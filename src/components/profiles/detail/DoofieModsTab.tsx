...
export function DoofieModsTab({
  ...
  const [doofieMods, setNoriskMods] = useState<DoofieMod[]>([]);
  ...
  useEffect(() => {
    if (profile.selected_doofie_pack_id && doofiePacksConfig) {
      fetchNoriskMods();
    } else {
      setNoriskMods([]);
      setIsLoading(false);
    }
  }, [profile.id, profile.selected_doofie_pack_id, doofiePacksConfig]);
  ...
  const fetchNoriskPacks = async () => {
    try {
      ...
      setError(
        `Failed to load Doofie packs: ${parseErrorMessage(error)}`,
      );
    }
  };
  ...
  const fetchNoriskMods = async () => {
    ...
    try {
      try {
        const modsResult = await invoke<any>("get_doofie_pack_mods", {
        ...
        if (!packDef) {
          setError(
            `Doofie pack "${profile.selected_doofie_pack_id}" not found. Try refreshing the packs.`,
          );
          setIsLoading(false);
          return;
        }
        ...
        if (
          ...
          try {
            const lastResortResult = await invoke<any>("list_doofie_mods", {
            ...
            if (lastResortResult && Array.isArray(lastResortResult)) {
              ...
              setError(
                "Could not load Doofie mods. No mods found in pack definition.",
              );
              setNoriskMods([]);
            }
          } catch (lastResortError) {
            setError(
              `Failed to load Doofie mods: ${
      ...
      setError(
        `Failed to load Doofie mods: ${parseErrorMessage(error)}`,
      );
    } finally {
      setIsLoading(false);
    }
  ...
  const handleToggleMod = async (modId: string) => {
    if (!profile.selected_doofie_pack_id) return;
  ...
  const handleRefresh = async () => {
    ...
    try {
      ...
      setError(
        `Failed to refresh Doofie packs: ${parseErrorMessage(error)}`,
      );
    } finally {
      setRefreshing(false);
    }
  ...
  const filteredMods = doofieMods.filter(
    (mod) =>
      mod.display_name
        .toLowerCase()
  ...
  const currentPackName = profile.selected_doofie_pack_id
    ? doofiePacksConfig?.packs[profile.selected_doofie_pack_id]?.displayName ||
      "Unknown Pack"
    : "No Pack Selected";

  const isExperimental = profile.selected_doofie_pack_id
    ? doofiePacksConfig?.packs[profile.selected_doofie_pack_id]
          ...
          <div className="h-full flex items-center justify-center">
            <div className="text-center">
              ...
              <p className="text-white/60 font-minecraft text-xl tracking-wide lowercase select-none">
                no doofie pack selected
              </p>
              <p className="text-white/40 font-minecraft text-sm mt-2 tracking-wide lowercase select-none">
                select a doofie pack in profile settings
              </p>
            </div>
          </div>
        ) : isLoading ? (
          <LoadingState message={t('content.doofie.loading')} />
        ) : error ? (
              ...
              <EmptyState
                ...
                message={
                  effectiveSearchQuery
                    ...
                    : "no doofie mods available"
                }
                description="Doofie mods are automatically managed by the launcher"
              />
            )}
          </ContentTable>
        )}
