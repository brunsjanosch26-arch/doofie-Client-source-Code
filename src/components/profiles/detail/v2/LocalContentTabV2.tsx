...
import type { NoriskModpacksConfig } from "../../../../types/doofiePacks";
...
const LOCAL_CONTENT_TAB_ICONS_TO_PRELOAD = [
  ...
  "solar:shield-cross-bold-duotone", // For Doofie blocked badge
];
...
export function LocalContentTabV2<T extends LocalContentItem>({
  ...
  // Fetch Flagsmith config when a Doofie pack is selected
  useEffect(() => {
    ...
    if (profile?.selected_doofie_pack_id) {
      FlagsmithService.getBlockedModsConfig()
        ...
        .catch((err) => {
          console.error("Failed to load Doofie blocked mods config:", err);
  ...
  // Fetch DoofiePacksConfig if content type is DoofieMod
  useEffect(() => {
    if (contentType === "DoofieMod" && profile) {
      const fetchPacks = async () => {
        ...
        try {
          ...
          console.error("Failed to fetch Doofie packs config:", err);
          toast.error(t('content.errors.load_doofie_packs'));
      ...
      setNoriskPacksConfig(null); // Clear if not DoofieMod or no profile
    }
  }, [contentType, profile]);

  const handleRefreshPacksList = useCallback(async () => {
    if (contentType !== "DoofieMod") return;
  ...
  const doofiePackOptions = useMemo((): SelectOption[] => {
    if (contentType !== "DoofieMod" || !doofiePacksConfig) {
  ...
  }, [contentType, doofiePacksConfig, t]);

  const handleSelectedPackChange = useCallback(
    async (newPackId: string | null) => {
      if (!profile || newPackId === profile.selected_doofie_pack_id) return;
      try {
        ...
        toast.error(
          `Failed to switch Doofie pack: ${parseErrorMessage(err)}`,
        );
      }
    },
  ...
  const renderListItem = useCallback(
    (item: T) => {
      ...
      const noRiskStatus = isBlockedConfigLoaded
        ? FlagsmithService.getModDoofieStatus(
            item.filename,
            item.modrinth_info?.project_id || item.curseforge_info?.project_id,
            item.modrinth_info?.version_id || item.curseforge_info?.file_id,
          )
        : null;
      const isBlockedByDoofie = noRiskStatus === 'blocked';
      const isWarningByDoofie = noRiskStatus === 'warning';
      
      // Debug logging
      if (noRiskStatus) {
        console.log('[LocalContentTabV2] Item:', item.filename, 'noRiskStatus:', noRiskStatus, 'isBlocked:', isBlockedByDoofie, 'isWarning:', isWarningByDoofie);
      }
      ...
      const itemIconNode = (
        <div className="absolute inset-0 w-full h-full flex items-center justify-center">
          ...
          {!isBlockedByDoofie && isWarningByDoofie && (
            <div className="absolute top-0.5 left-0.5 z-10 pointer-events-auto">
      ...
      const itemBadgesNode = [
        // Doofie crash warning (highest priority)
        ...(isBlockedByDoofie ? [{
        ...
        ...(itemPlatform !== 'Local' ? [{
          icon: itemPlatform === 'Modrinth'
            ...
            : "https://cdn.doofie.gg/misc/curseforge.webp",
          text: itemPlatform,
      ...
      // Note: Doofie mods can also have updates available, but they won't be auto-updatable
      ...
      if (isCurrentlyUpdating && !item.doofie_info) {
        itemActions.push({
          id: "updating",
      ...
      // Delete action (if not Doofie mod) - icon-only
      if (!item.doofie_info) {
        itemActions.push({
          id: "delete",
    ...
    <div className="flex flex-col flex-grow min-w-0">
      ...
      <>
        <div className="flex items-center justify-between w-full min-h-14">
          ...
          {/* Right side: Action Buttons and DoofiePack Dropdown */}
          <div className="flex items-center gap-2">
            ...
            {selectedItemIds.size === 0 && (
              <>
                {/* Doofie Pack Selector - Only for DoofieMod type */}
                {contentType === "DoofieMod" &&
                  doofiePacksConfig &&
                  doofiePackOptions.length > 0 && (
                    <div className="flex items-center gap-2">
                      <Select
                ...
                {/* Update All buttons - Only for non-DoofieMod types */}
                {contentType !== "DoofieMod" && updatableContentCount > 0 && (
                  <ContentActionButtons
                    actions={[
                      {
                        id: "update-all",
                ...
                {/* Browse and Add buttons - only for non-DoofieMod types */}
                {effectiveOnAddContent && contentType !== "DoofieMod" && profile && (
                  <ContentActionButtons
                    actions={[
                      {
                        id: "browse",
  ...
  const showDoofiePackSelector = contentType === "DoofieMod";
  const isDoofiePackSelected =
    showDoofiePackSelector && profile?.selected_doofie_pack_id;

  // Dynamic empty state messages
  const getEmptyStateMessage = () => {
    if (contentType === "DoofieMod" && !profile?.selected_doofie_pack_id) {
  ...
  return (
    <>
      <GenericContentTab<T>
        items={
          contentType === "DoofieMod" && !profile?.selected_doofie_pack_id
            ? []
            : filteredItems
        }
        ...
        emptyStateAction={
          // Show browse button for empty states (except when Doofie pack not selected and when loading)
          (isTrulyEmptyState ||
