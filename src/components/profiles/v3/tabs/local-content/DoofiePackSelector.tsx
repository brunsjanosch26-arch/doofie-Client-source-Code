 ...
 *
 * Owns: `doofiePacksConfig`-Fetch, `packMenuOpen`, Refresh-Zustand.
 * Parent-API: nur `profile` + `onChanged`. Der Parent refresht seine Mod-Liste
 * via `onChanged` sobald sich `selected_doofie_pack_id` aendert.
 *
...
export function NoriskPackSelector({ profile, onChanged }: NoriskPackSelectorProps) {
  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        ...
        console.error("[V3] Failed to fetch Doofie packs config:", err);
  ...
  const handleChange = useCallback(async (newPackId: string | null) => {
    ...
    try {
      ...
      console.error("[V3] Failed to switch Doofie pack:", err);
  ...
  const handleRefreshList = useCallback(async () => {
    ...
    try {
      ...
      console.error("[V3] Failed to refresh Doofie packs:", err);
