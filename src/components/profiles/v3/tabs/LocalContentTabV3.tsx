 ...
 * Shaders, DataPacks, Doofie. Wiederverwendet useLocalContentManager (identische
 ...
 * kein Doofie-Pack-Selector. Komplexere Operationen bleiben im V2-Tab
...
import { NoriskPackSelector } from "./local-content/DoofiePackSelector";
...
type FilterKey =
  ...
  | "hasUpdate" | "fromModpack" | "manuallyAdded" | "updatesPaused" | "doofieIssues";
...
const FILTER_OPTIONS: FilterOption[] = [
  ...
  { value: "doofieIssues",  labelKey: "profiles.v3.filter.doofieIssues",   icon: "solar:danger-triangle-bold", separator: true, nrcOnly: true },
];
...
export function LocalContentTabV3<T extends LocalContentItem>({
  ...
  const isNrc = contentType === "DoofieMod";
  const selectedPackId = profile?.selected_doofie_pack_id ?? null;

  // Flagsmith blocked-mods Config laden wenn ein NRC-Pack aktiv ist.
  // Beeinflusst die Warn-Overlays auf Mods die mit Doofie inkompatibel sind.
  ...
  useEffect(() => {
    if (profile?.selected_doofie_pack_id) {
      FlagsmithService.getBlockedModsConfig()
        ...
        .catch((err) => {
          console.error("[V3] Failed to load Doofie blocked mods config:", err);
  ...
  const visibleItems = useMemo(() => {
    ...
    switch (filter) {
      ...
      case "manuallyAdded": list = list.filter(i => !i.modpack_origin && !i.doofie_info); break;
      case "updatesPaused": list = list.filter(i => i.updates_enabled === false); break;
      case "doofieIssues":  list = list.filter(i => {
        if (!isBlockedConfigLoaded) return false;
        const status = FlagsmithService.getModDoofieStatus(
          i.filename,
  ...
  // Doofie mods are gated off the sheet since they come via the pack
  // selector, not Modrinth browse.
  const canBrowse = contentType !== "DoofieMod";
  ...
  return (
    <div className="flex flex-col min-h-0 flex-1 relative">
      ...
      <div className="flex items-center gap-3 px-5 py-2.5 border-b border-white/10 flex-shrink-0 bg-black/20 sticky top-0 z-10">
        ...
        {/* Doofie pack selector (only for NRC) */}
        {isNrc && (
