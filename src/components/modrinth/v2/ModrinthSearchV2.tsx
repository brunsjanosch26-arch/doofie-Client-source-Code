...
import { getBlockedModsConfig, getModDoofieStatus } from '../../../services/flagsmith-service';
import type {
  ModrinthProjectType,
...
export function ModrinthSearchV2({
  ...
  // Load blocked mods config on mount (cached from doofie-service if already loaded)
  useEffect(() => {
  ...
  const loadProjectVersions = async (projectId: string) => {
    ...
    try {
      ...
      // Add Doofie status to each version
      const versionsWithDoofieStatus = response.versions.map(version => {
        const primaryFile = version.files.find(file => file.primary) || version.files[0];
        const filename = primaryFile?.filename || '';
        const noRiskStatus = getModDoofieStatus(filename, projectId, version.id);
      ...
      const sortedVersions = versionsWithDoofieStatus.sort((a, b) => new Date(b.date_published).getTime() - new Date(a.date_published).getTime());
  ...
  const checkDisplayedVersionsStatus = async (projectId: string, versions: UnifiedVersion[], startIndex: number, count: number, forceRefresh: string[] = []) => {
    ...
    try {
      ...
      // First request for just the project to check Doofie pack status
      requests.push({
        project_id: projectId,
      ...
      let projectInDoofieStatus: ContentInstallStatus | null = null;
      
      batchResults.results.forEach(result => {
        if (result.request_id === `project-${projectId}`) {
          // This is the project-level check for Doofie pack
          projectInDoofieStatus = result.status;
        } else if (result.request_id) {
          // This is a version check
          newInstalledState[result.request_id] = {
            ...result.status,
            // If project is in Doofie pack, set is_included_in_doofie_pack based on version match
            is_included_in_doofie_pack: 
              projectInDoofieStatus?.is_included_in_doofie_pack && result.status.is_specific_version_in_pack
          };
        }
      });
      ...
      try {
        const projectInDoofieStatus = await ProfileService.isContentInstalled({
          profile_id: selectedProfile.id,
          project_id: projectId,
          project_type: projectType
        });
          ...
          try {
            ...
            if (!primaryFile) {
              newInstalledState[version.id] = {
                is_installed: false,
                is_included_in_doofie_pack: false,
                is_specific_version_in_pack: false,
                is_enabled: null,
                found_item_details: null,
                doofie_pack_item_details: null,
              };
              continue;
            }
            ...
            newInstalledState[version.id] = {
              ...
              is_included_in_doofie_pack: projectInDoofieStatus.is_included_in_doofie_pack && statusFromService.is_specific_version_in_pack,
              ...
              doofie_pack_item_details: statusFromService.doofie_pack_item_details || null,
            };
          } catch (error) {
        ...
        success: (data: any) => {
          ...
          setInstalledVersions(prev => {
            ...
            newState[profileId][version.id] = { // Use profileId
              is_installed: false,
              is_included_in_doofie_pack: newState[profileId]?.[version.id]?.is_included_in_doofie_pack || false, // Use profileId
              is_specific_version_in_pack: newState[profileId]?.[version.id]?.is_specific_version_in_pack || false, // Use profileId
              is_enabled: null,
              found_item_details: null,
              doofie_pack_item_details: newState[profileId]?.[version.id]?.doofie_pack_item_details || null, // Use profileId
            };
          ...
          if (!anyVersionsStillInstalled && selectedProfile && selectedProfile.id === profileId) {
            setInstalledProjects(prev => ({
              ...
              [project.project_id]: {
                is_installed: false,
                is_included_in_doofie_pack: prev[project.project_id]?.is_included_in_doofie_pack || false,
                is_specific_version_in_pack: prev[project.project_id]?.is_specific_version_in_pack || false,
                is_enabled: null,
                found_item_details: null,
                doofie_pack_item_details: prev[project.project_id]?.doofie_pack_item_details || null,
              }
            }));
          }
  ...
  const handleToggleEnableVersion = async (
    ...
    // Check if this is a Doofie Pack item
    if (currentVersionStatus?.doofie_pack_item_details?.doofie_mod_identifier) {
      const doofieIdentifier = currentVersionStatus.doofie_pack_item_details.doofie_mod_identifier;
      ...
      await toast.promise(
        async () => {
          const payload: ToggleContentPayload = {
            ...
            doofie_mod_identifier: doofieIdentifier,
            content_type: nrContentType, // Pass content_type here as well
            // sha1_hash is not strictly needed for doofie_mod_identifier-based toggling by current backend logic,
          ...
          setInstalledVersions(prev => {
            ...
            if (newState[selectedProfile.id][version.id]) {
              newState[selectedProfile.id][version.id] = {
                ...
                doofie_pack_item_details: {
                  ...newState[selectedProfile.id][version.id]!.doofie_pack_item_details!,
                  is_enabled: newEnabledState
                }
              };
            }
              ...
              return {
                ...
                [project.project_id]: {
                  ...
                  doofie_pack_item_details: {
                    ...
                    ...(currentProjectStatus.doofie_pack_item_details || {}),
                    // We might not have a full doofie_mod_identifier here at project level,
        ...
        {
          loading: `${toastMessage} Doofie Pack item: ${project.title} (${version.version_number})...`,
          success: ({ versionName }) => `Successfully ${successMessage} Doofie Pack item: ${project.title} (${versionName})`,
          error: (err) => `Failed to ${toastMessage.toLowerCase()} Doofie Pack item: ${err.message || parseErrorMessage(err)}`
        }
      ).catch(err => {
        console.error(`Error ${toastMessage.toLowerCase()} Doofie Pack item:`, err);
      });
      
      return; // Exit after handling Doofie pack item
    }

    // Regular content toggle using SHA1 hash (for non-Doofie pack items)
    if (!sha1Hash) {
    ...
    await toast.promise(
      async () => {
        const payload: ToggleContentPayload = {
          ...
          doofie_mod_identifier: undefined, // Explicitly undefined for non-Doofie items
        };
  ...
  const getStatusForNewInstall = (
    ...
    is_installed: true,
    is_included_in_doofie_pack: existingPreviousStatus?.is_included_in_doofie_pack || false,
    ...
    doofie_pack_item_details: existingPreviousStatus?.doofie_pack_item_details || null,
  });

  // Helper function to get Doofie status for a project
  const getProjectDoofieStatus = (project: UnifiedModSearchResult): 'blocked' | 'warning' | null => {
    console.log('[getProjectDoofieStatus] Checking project:', project.title, 'ID:', project.project_id);
    console.log('[getProjectDoofieStatus] Config loaded:', blockedModsConfigLoaded);
    
    if (!blockedModsConfigLoaded) {
      console.log('[getProjectDoofieStatus] Config not loaded yet, returning null');
      return null;
    }
    
    const result = getModDoofieStatus('', project.project_id, null);
    console.log('[getProjectDoofieStatus] Result for', project.project_id, ':', result);
    return result;
  };
  ...
  const isProjectBlocked = (project: UnifiedModSearchResult): boolean => {
    return getProjectDoofieStatus(project) === 'blocked';
  };
            ...
            disableVirtualization ? (
              ...
              <div className="space-y-1">
                {searchResults.map((hit, index) => {
                  ...
                  const projectDoofieStatus = getProjectDoofieStatus(hit);
                  ...
                  return (
                    <ModrinthProjectCardV2
                      ...
                      projectDoofieStatus={projectDoofieStatus}
