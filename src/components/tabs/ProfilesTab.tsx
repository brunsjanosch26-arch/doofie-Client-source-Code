...
export function ProfilesTab() {
  ...
  const groupedProfiles = (() => {
    // Separate Doofie Client profiles and all other profiles
    const doofieClientProfiles = initiallyFilteredProfiles.filter(profile => 
    ...
    // Sort both groups
    doofieClientProfiles.sort((a, b) => a.name.localeCompare(b.name));
    otherProfiles.sort((a, b) => a.name.localeCompare(b.name));
    
    const result: Record<string, Profile[]> = {};
    
    // Always add Doofie Client group first if there are profiles with that group
    if (doofieClientProfiles.length > 0) {
      result["Doofie Client"] = doofieClientProfiles;
    }
  ...
  const sortedGroupKeys = Object.keys(groupedProfiles).sort((a, b) => {
    // Doofie Client should always be first
    if (a === "Doofie Client") return -1;
    if (b === "Doofie Client") return 1;
