...
export function CapeBrowser(): JSX.Element {
  ...
  const equippedCapeId = useMemo(() => {
    ...
    // For Doofie capes, we don't have equipped state yet
    return null;
  ...
  const capesData = useMemo(() => {
    ...
    // When searching Doofie capes, always show search results from allCapes
    if (searchQuery && searchQuery.trim() !== "") {
      return allCapes;
    }
  ...
  const handleEquipCape = async (capeHash: string) => {
    ...
    if (filters.showVanillaOnly) {
      ...
      // For Doofie capes, use the regular equip function
