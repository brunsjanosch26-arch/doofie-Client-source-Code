...
import { refreshNrcDataOnMount } from "./services/doofie-service";
import {
  getLauncherConfig,
...
export function App() {
  ...
  useEffect(() => {
    ...
    const storedTheme = localStorage.getItem("doofie-theme-storage");
    if (storedTheme) {
      try {
  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        ...
        if (cancelled) return;
        const allowed = await hasPermission("doofie.tester");
