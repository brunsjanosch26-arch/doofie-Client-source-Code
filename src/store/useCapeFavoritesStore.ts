...
interface CapeFavoritesState {
  ...
  clearFavorites: () => void;
  setFavoriteOptimistic: (capeId: string, favorite: boolean, doofieToken?: string) => Promise<void>;
  toggleFavoriteOptimistic: (capeId: string, doofieToken?: string) => Promise<void>;
}

const STORAGE_KEY = "doofie-cape-favorites";
...
export const useCapeFavoritesStore = create<CapeFavoritesState>()(
  persist(
    (set, get) => ({
      ...
      setFavoriteOptimistic: async (capeId: string, favorite: boolean, doofieToken?: string) => {
        ...
        try {
          ...
          await setCapeFavorite(capeId, favorite, doofieToken);
        } catch (err) {
      ...
      toggleFavoriteOptimistic: async (capeId: string, doofieToken?: string) => {
        const nextFavorite = !get().isFavorite(capeId);
        await get().setFavoriteOptimistic(capeId, nextFavorite, doofieToken);
      },
    }),
    {
      name: STORAGE_KEY,
