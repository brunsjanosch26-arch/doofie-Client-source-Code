...
export type LocalContentType = 'ShaderPack' | 'ResourcePack' | 'DataPack' | 'Mod' | 'DoofieMod';
...
function mapUiContentTypeToBackend(uiType: LocalContentType): NrContentType {
  switch (uiType) {
    ...
    case 'DoofieMod': return NrContentType.DoofieMod;
...
function mapBackendItemToFrontendType<T extends LocalContentItem>(rawItem: ProfileLocalContentItem): T {
  ...
  // It has fields like: filename, path_str, ..., doofie_identifier, fallback_version
  // The actual object from Rust via invoke might have `doofie_info` field instead of `doofie_identifier` being populated.
  const outputItem = {
    ...rawItem, // Spread all properties from rawItem (which is typed as ProfileLocalContentItem)
    path: rawItem.path_str, // Add/override path using path_str from ProfileLocalContentItem // Fallback to the typed doofie_identifier if doofie_info isn't there
  };
...
function createUninstallPayload<T extends LocalContentItem>(
  ...
  if (uiContentType === 'Mod') {
    ...
    return { profile_id: profileId, file_path: item.path };
  } else if (uiContentType === 'DoofieMod') {
    toast.error(i18n.t('content_manager.errors.doofie_uninstall_not_supported'));
    console.error("[useLocalContentManager] Attempted to create uninstall payload for DoofieMod. This is generally not supported here.");
    return null;
  }
...
function createTogglePayload<T extends LocalContentItem>(
  ...
  const payloadBase: Omit<ToggleContentPayload, 'sha1_hash' | 'file_path' | 'doofie_mod_identifier'> = {
    profile_id: profileId,
    enabled: targetEnabledState,
    content_type: backendContentType,
  };

  if (uiContentType === 'DoofieMod') {
    const doofieIdentifierFromItem = (item as ProfileLocalContentItem).doofie_info; // Expect doofie_info from the item
    if (doofieIdentifierFromItem) {
      return { ...payloadBase, doofie_mod_identifier: doofieIdentifierFromItem }; // Map to payload's doofie_mod_identifier
    } else {
      toast.error(i18n.t('content_manager.errors.doofie_missing_info_toggle', { filename: item.filename }));
      return null;
    }
...
export function useLocalContentManager<T extends LocalContentItem>({
  ...
  }, [fetchBasicInfo, profile?.selected_doofie_pack_id]); // Added profile.selected_doofie_pack_id to ensure refetch on pack change
    ...
    const fetchLocalArchiveIcons = async () => {
      ...
      const pathsToFetchIconsFor = items
        .filter(item => {
          ...
          // For DoofieMod, the item.path points to a .jar file in cache
          // For other types, item.path usually points to a .zip file
          const lowerPath = item.path.toLowerCase();
          if (contentType === 'DoofieMod') {
  ...
  const handleToggleItemEnabled = useCallback(async (item: T) => {
    ...
    try {
      ...
      if (contentType !== 'DoofieMod' && onRefreshRequiredRef.current) {
  ...
  const checkForContentUpdates = useCallback(async (currentProfile = profile, currentItems = items) => {
    ...
    try {
      const request: UnifiedUpdateCheckRequest = {
        hashes,
        algorithm: "sha1",
        loaders: (contentType === 'Mod' || contentType === 'DoofieMod') && currentProfile.loader ? [currentProfile.loader] : [],
  ...
  const handleUpdateContentItem = useCallback(async (item: T, updateVersion: UnifiedVersion, suppressOwnToast: boolean = false) => {
    ...
    if (!item.path && !(contentType === 'Mod' && item.id && !item.source_type && !item.doofie_info)) {
      toast.error(i18n.t('content_manager.errors.item_path_missing_update', { name: getDisplayFileName(item) }));
      return;
    }

    // Platform-specific validation
    if (platform === 'Modrinth' && contentType === 'Mod' && item.id && !item.source_type && !item.doofie_info && !item.modrinth_info) {
      toast.error(i18n.t('content_manager.errors.mod_not_modrinth', { name: getDisplayFileName(item) }));
      return;
    }

    if (platform === 'CurseForge' && contentType === 'Mod' && item.id && !item.source_type && !item.doofie_info && !item.curseforge_info) {
