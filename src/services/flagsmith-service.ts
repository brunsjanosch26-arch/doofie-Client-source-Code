...
const initializeFlagsmith = async () => {
  try {
    ...
    await flagsmith.init({
      ...
      api: 'https://flagsmith-staging.doofie.gg/api/v1/',
      identity: installId,
    });
...
export const getPackFallbackConfig = async (): Promise<PackFallbackConfig | null> => {
  ...
  packFallbackFetchPromise = (async () => {
    try {
      ...
      // Flag unset → leave the backend default ("doofie-stable") untouched.
...
/**
 * Returns the Doofie status of a mod based on the cached config.
 ...
 */
export const getModDoofieStatus = (
  filename: string,
  modrinthProjectId?: string | null,
  versionId?: string | null,
): 'blocked' | 'warning' | null => {
  console.log('[getModDoofieStatus] Called with filename:', filename, 'projectId:', modrinthProjectId, 'versionId:', versionId, 'cachedConfig:', cachedBlockedModsConfig);

  if (!cachedBlockedModsConfig) {
    console.log('[getModDoofieStatus] Config not cached, returning null');
    // Silently return null if config is not loaded. The UI should trigger the load.
    return null;
  }

  const config = cachedBlockedModsConfig;
  console.log('[getModDoofieStatus] Checking against config:', config);

  // 1. Check exact filename match (blocked)
  if (config.exact_filenames?.includes(filename)) {
    console.log('[getModDoofieStatus] MATCHED exact filename - BLOCKED!');
    return 'blocked';
  }

  // 2. Check Modrinth project ID for blocking
  if (modrinthProjectId && config.modrinth_project_ids?.includes(modrinthProjectId)) {
    console.log('[getModDoofieStatus] MATCHED Modrinth project ID - BLOCKED!');
    return 'blocked';
  }

  // 3. Check version ID (mod_ids) (blocked)
  if (versionId && config.mod_ids?.includes(versionId)) {
    console.log('[getModDoofieStatus] MATCHED version ID (mod_id) - BLOCKED!');
    return 'blocked';
  }
    ...
    for (const pattern of config.filename_patterns) {
      try {
        ...
        if (regex.test(filename)) {
          console.log('[getModDoofieStatus] MATCHED filename pattern - BLOCKED!', pattern);
          return 'blocked';
        }
      } catch (e) {
  ...
  if (modrinthProjectId && config.warning_project_ids?.includes(modrinthProjectId)) {
    console.log('[getModDoofieStatus] MATCHED Modrinth project ID - WARNING!');
    return 'warning';
  }

  console.log('[getModDoofieStatus] No match found, returning null');
  return null;
};