...
pub fn mask_sensitive_data(content: &str) -> String {
    lazy_static! {
        // Mask Doofie client tokens
        static ref NORISK_TOKEN_REGEX: Regex = Regex::new(r"-Ddoofie\.token=[^\s]+").unwrap();
    ...
    let mut masked_content = NORISK_TOKEN_REGEX.replace_all(content, "-Ddoofie.token=*****").to_string();
