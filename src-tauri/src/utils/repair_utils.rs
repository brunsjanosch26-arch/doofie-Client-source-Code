...
pub async fn repair_profile_mods(profile_id: Uuid) -> Result<()> {
    ...
    if mod_cache_dir.exists() {
        ...
        // 3. Delete Doofie pack mod files from cache
        if let Some(pack_id) = profile.effective_doofie_pack_id().await {
            info!("Cleaning Doofie pack mod cache for pack: {}", pack_id);

            let doofie_config = state.doofie_pack_manager.get_config().await;
            match doofie_config.get_resolved_pack_definition(&pack_id) {
                Ok(resolved_pack) => {
                    for doofie_mod in &resolved_pack.mods {
                        // Get the cache path for this Doofie mod
                        match path_utils::get_doofie_mod_cache_path(
                            doofie_mod,
                            ...
                            Ok(cache_path) => {
                                if cache_path.exists() {
                                    match fs::remove_file(&cache_path).await {
                                        Ok(_) => {
                                            debug!("Removed Doofie pack mod cache file: {:?}", cache_path);
                                            cache_files_removed += 1;
                                        }
                                        Err(e) => {
                                            warn!("Failed to remove Doofie pack mod cache file {:?}: {}", cache_path, e);
                                            cache_errors += 1;
                                        }
                                    }
                                } else {
                                    debug!("Doofie pack mod cache file does not exist: {:?}", cache_path);
                                }
                            }
                            Err(e) => {
                                warn!("Could not determine cache path for Doofie pack mod {}: {}", 
                                      doofie_mod.display_name.as_deref().unwrap_or(&doofie_mod.id), e);
                                cache_errors += 1;
                            }
                        }
                    }
                ...
                Err(e) => {
                    warn!("Failed to get Doofie pack definition for {}: {}", pack_id, e);
                    cache_errors += 1;
                }
            }
        } else {
            debug!("Profile has no selected Doofie pack, skipping Doofie mod cache cleanup");
        }
