...
use crate::integrations::doofie_packs;
...
async fn get_content_directory(profile: &Profile, content_type: &ContentType) -> Result<PathBuf> {
    match content_type {
        ...
        ContentType::DoofieMod => {
            // DoofieMods don't have a physical directory but we return a path for consistency
            let state = State::get().await?;
            let instance_path = state
                .profile_manager
                .calculate_instance_path_for_profile(profile)?;
...
fn content_type_to_string(content_type: &ContentType) -> &'static str {
    match content_type {
        ...
        ContentType::Mod => "Mod",
        ContentType::DoofieMod => "Doofie Mod",
    }
}
...
/// Represents details about an item when it comes from a Doofie Pack
#[derive(Serialize, Deserialize, Debug, Clone)]
pub struct DoofiePackItemDetails {
    pub is_enabled: bool,
    pub doofie_mod_identifier: Option<crate::state::profile_state::NoriskModIdentifier>,
}

#[derive(Serialize, Debug, Default, Clone, Deserialize)]
pub struct ContentInstallStatus {
    pub is_included_in_doofie_pack: bool,
    pub is_installed: bool,
    pub is_specific_version_in_pack: bool,
    pub is_enabled: Option<bool>,
    pub found_item_details: Option<FoundItemDetails>,
    pub doofie_pack_item_details: Option<DoofiePackItemDetails>,
}
...
pub async fn check_content_installed(params: CheckContentParams) -> Result<ContentInstallStatus> {
    ...
    if let Some(pack_id) = profile.effective_doofie_pack_id().await {
        debug!(
        ...
        match config.get_resolved_pack_definition(&pack_id) {
            Ok(resolved_pack) => {
                for doofie_mod in &resolved_pack.mods {
                    let mut is_potential_project_match = false;
                    if let (
                        Some(pid_arg),
                        doofie_packs::NoriskModSourceDefinition::Modrinth {
                            project_id: doofie_pid,
                            ..
                        },
                    ) = (&params.project_id, &doofie_mod.source)
                    {
                        if pid_arg == doofie_pid {
                    ...
                    if is_potential_project_match {
                        if let Some(loader_map) = doofie_mod.compatibility.get(target_game_version)
                        {
                            if let Some(target) = loader_map.get(target_loader_str) {
                                status.is_included_in_doofie_pack = true;
                                ...
                                // New addition: Add DoofiePackItemDetails
                                let mod_identifier = doofie_mod.id.clone();

                                // Create a proper NoriskModIdentifier
                                let doofie_mod_identifier =
                                ...
                                let is_pack_mod_enabled = !profile
                                    .disabled_doofie_mods_detailed
                                    .contains(&doofie_mod_identifier);

                                status.doofie_pack_item_details = Some(DoofiePackItemDetails {
                                    is_enabled: is_pack_mod_enabled,
                                    doofie_mod_identifier: Some(doofie_mod_identifier),
                                });
                ...
                if status.is_included_in_doofie_pack {
...
/// Exports a profile to a `.doofiepack` file
///
/// This creates a zip archive with the .doofiepack extension that contains:
...
/// @param output_path: Optional path where the .doofiepack file should be saved
/// @param include_files: Optional list of files/directories to include in the overrides folder
/// @return: Result containing the path to the created .doofiepack file
pub async fn export_profile_to_doofiepack(
    profile_id: Uuid,
    output_path: Option<PathBuf>,
    include_files: Option<Vec<PathBuf>>,
) -> Result<PathBuf> {
    info!("Exporting profile {} to .doofiepack", profile_id);
    ...
    info!("Exporting {} files to .doofiepack", total_files);
    ...
    info!("Creating .doofiepack archive at: {}", output_file.display());
...
async fn process_mod_requests(
    ...
    // For each request, we need to check both in Doofie Pack and local installation
    for (request, idx) in requests {
        // Convert to the old params format for reusing doofie pack check logic
        let old_params = CheckContentParams {
            profile_id: profile.id,
            project_id: request.project_id.clone(),
            version_id: request.version_id.clone(),
        ...
        // Check if included in Doofie Pack
        if let Some(pack_id) = profile.effective_doofie_pack_id().await {
            let state = State::get().await?;
            let config = state.doofie_pack_manager.get_config().await;
            ...
            if let Ok(resolved_pack) = config.get_resolved_pack_definition(&pack_id) {
                for doofie_mod in &resolved_pack.mods {
                    ...
                    ) = (&request.project_id, &doofie_mod.source)
                    {
                        if pid_arg == doofie_pid {
                            is_potential_project_match = true;
                        }
...
async fn process_resourcepack_requests(
    ...
    for (request, idx) in requests {
        ...
        // Check Doofie Pack - reuse old function for now
        let old_params = CheckContentParams {
            profile_id: profile.id,
            project_id: request.project_id.clone(),
            version_id: request.version_id.clone(),
        ...
        // Check if in Doofie Pack
        if let Some(pack_id) = &profile.selected_doofie_pack_id {
            let state = State::get().await?;
            let config = state.doofie_pack_manager.get_config().await;

            if let Ok(resolved_pack) = config.get_resolved_pack_definition(pack_id) {
                // Check if the pack includes this resource pack
                // (Note: This would need to be expanded if Doofie Packs can contain resource packs)
...
async fn process_shaderpack_requests(
    ...
    for (request, idx) in requests {
        ...
        // Check if in Doofie Pack - placeholder for future Doofie Pack shader support
        if let Some(pack_id) = &profile.selected_doofie_pack_id {
...
async fn process_datapack_requests(
    ...
    for (request, idx) in requests {
        ...
        // Check if in Doofie Pack - placeholder for future Doofie Pack datapack support
        if let Some(pack_id) = &profile.selected_doofie_pack_id {
            // Placeholder for future implementation
        }
...
pub struct LocalContentItem {
    ...
    pub doofie_info: Option<crate::state::profile_state::NoriskModIdentifier>, // Identifier für DoofieMods
...
impl LocalContentLoader {
    ...
    pub async fn load_items(
        ...
        let content_dirs = match params.content_type {
            ...
            ContentType::DoofieMod => {
                // For Doofie mods, handled differently (no physical directory scan)
                Vec::new()
            }
        };
        ...
        if params.content_type == ContentType::DoofieMod {
            // Special handling for Doofie mods - fetch them from the Doofie pack system
            if let Some(pack_id) = profile.effective_doofie_pack_id().await {
                // Get the Doofie pack manager from the state
                let state = State::get().await?;
                let config = state.doofie_pack_manager.get_config().await;
                    ...
                    Ok(pack_def) => {
                        for doofie_mod in &pack_def.mods {
                            ...
                            let is_disabled = profile
                                .disabled_doofie_mods_detailed
                                .iter()
                                .any(|disabled_mod| *disabled_mod == doofie_mod_identifier);

                            // Determine source type string
                            let source_type_str = match &doofie_mod.source {
                                crate::integrations::doofie_packs::NoriskModSourceDefinition::Modrinth { .. } => None,
                                crate::integrations::doofie_packs::NoriskModSourceDefinition::Maven { .. } => Some("maven"),
                                crate::integrations::doofie_packs::NoriskModSourceDefinition::Url { .. } => Some("url"),
                                _ => Some("doofie"),
                            };

                            // Extract Modrinth info if available
                            let modrinth_info = if let crate::integrations::doofie_packs::NoriskModSourceDefinition::Modrinth { project_id, .. } = &doofie_mod.source {
                                // For version info we need to look at compatibility
                                let version_id = doofie_mod.compatibility
                                    .get(&profile.game_version)
                                ...
                                Some(GenericModrinthInfo {
                                    ...
                                    version_id,
                                    name: doofie_mod.display_name.clone().unwrap_or_else(|| doofie_mod.id.clone()),
                            ...
                            let path_str = match crate::utils::path_utils::get_doofie_mod_cache_path(
                                doofie_mod,
                                &profile.game_version,
                                &profile.loader.as_str(),
                            ) {
                                ...
                                Err(e) => {
                                    warn!(
                                        "Could not get cache path for Doofie mod {}: {}",
                                        doofie_mod.id, e
                                    );
                            ...
                            preliminary_items.push(LocalContentItem {
                                filename: doofie_mod.id.clone(),
                                path_str,
                                sha1_hash: None,
                                file_size: 0,
                                is_disabled,
                                is_directory: false,
                                content_type: ContentType::DoofieMod,
                                modrinth_info,
                                curseforge_info: None,
                                platform: None,
                                source_type: source_type_str.map(|s| s.to_string()),
                                doofie_info: Some(doofie_mod_identifier),
                                fallback_version: fallback_version,
                                id: None,
                                associated_loader: None,
                                modpack_origin: None, // Doofie mods kommen nicht aus ModPacks
                                updates_enabled: None, // Default behavior
                            });
                        }
                    }
                    Err(e) => {
                        warn!("Failed to get Doofie pack definition: {}", e);
                    }
                }
            }
        ...
        for content_dir in &content_dirs {
                ...
                entries.next_entry().await.map_err(|e| AppError::Io(e))?
            {
                ...
                let is_valid_item = match params.content_type {
                    ...
                    ContentType::DoofieMod => false, // We handle Doofie mods differently, not by scanning directories
                };
        ...
        // If the content type is DoofieMod, sort the items by filename for consistent ordering
        if params.content_type == ContentType::DoofieMod {
        ...
        for (idx, item) in final_items.iter().enumerate() {
            info!(
                "Final item [{}]: filename='{}', path_str='{}', sha1_hash={:?}, file_size={}, is_disabled={}, is_directory={}, content_type={:?}, source_type={:?}, doofie_info={:?}, id={:?}, associated_loader={:?}, fallback_version={:?}, modrinth_info={:?}",
                idx,
                item.filename,
                item.path_str,
                item.sha1_hash,
