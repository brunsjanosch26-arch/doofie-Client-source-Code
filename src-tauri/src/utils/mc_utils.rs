...
/// Copies StartUpHelper data from the doofieclient/new directory to a new profile's directory.
...
pub async fn copy_startup_helper_data(
    ...
    profile_dir: &PathBuf,
    doofie_pack: Option<&crate::integrations::doofie_packs::NoriskPackDefinition>,
) -> Result<()> {
    ...
    if let Err(e) = copy_startup_helper_files(profile, profile_dir, doofie_pack).await {
...
/// Copies additional files specified in StartUpHelper from doofieclient/new/ directory
...
pub async fn copy_startup_helper_files(
    ...
    let startup_helper = match doofie_pack {
    ...
    // Get the doofieclient/new directory path
    let default_profile_path = crate::state::profile_state::default_profile_path();
    let doofie_dir = default_profile_path
        .join("doofieclient")
        .join("new");

    if !doofie_dir.exists() {
        info!(
            "[{}] DoofieClient new directory not found at: {}, skipping StartUpHelper.",
            profile_id,
            doofie_dir.display()
        );
        return Ok(());
    ...
    info!(
        "[{}] Found DoofieClient new directory at: {}",
        profile_id,
        doofie_dir.display()
    );
    ...
    for relative_path in &startup_helper.additional_paths {
        let src_path = doofie_dir.join(relative_path);
