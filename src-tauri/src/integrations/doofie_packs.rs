...
/// Represents the overall structure of the doofie_modpacks.json file.
#[derive(Serialize, Deserialize, Debug, Clone)]
pub struct NoriskModpacksConfig {
    /// A map where the key is the pack ID (e.g., "doofie-prod") and the value is the pack definition.
    pub packs: HashMap<String, NoriskPackDefinition>,
    /// A map defining Maven repositories used by mods with source type "maven".
    /// Key is a reference name (e.g., "doofieproduction"), value is the repository URL.
...
/// Configuration for copying additional files from the doofieclient/new directory
/// to new profiles when using this modpack.
#[derive(Serialize, Deserialize, Clone, Debug)]
pub struct StartUpHelper {
    /// List of relative paths to copy from doofieclient/new/ to the profile directory.
...
pub fn get_doofie_pack_mod_filename(
...
/// Imports a profile from a .doofiepack file.
...
pub async fn import_doofiepack_as_profile(pack_path: PathBuf, event_id: Option<Uuid>) -> Result<Uuid> {
    info!("Starting import process for doofiepack: {:?}", pack_path);
    ...
    let profile_json_file = File::open(&pack_path).await.map_err(|e| {
        error!(
            "Failed to open doofiepack file for profile.json {:?}: {}",
            pack_path, e
        );
        AppError::Io(e)
    })?;
    ...
    let mut zip_for_profile_json = ZipFileReader::with_tokio(&mut profile_json_buf_reader)
        .await
        .map_err(|e| {
            error!("Failed to read doofiepack as ZIP for profile.json: {}", e);
            AppError::Other(format!(
                "Failed to read doofiepack zip for profile.json: {}",
                e
            ))
        })?;
    ...
    if let Some(file_name) = pack_path.file_stem().and_then(|s| s.to_str()) {
        info!("Using doofiepack filename as profile name: {}", file_name);
    ...
    let final_profile_name = if sanitized_base_name.is_empty() {
        let default_name = format!("imported-doofiepack-{}", Utc::now().timestamp_millis());
        warn!(
    ...
    let overrides_file_for_listing = File::open(&pack_path).await.map_err(|e| {
        error!(
            "Failed to open doofiepack file for overrides listing {:?}: {}",
            pack_path, e
        );
        AppError::Io(e)
    })?;
    ...
    let mut zip_lister_for_overrides = ZipFileReader::with_tokio(&mut overrides_buf_reader)
        ...
        .map_err(|e| {
        error!(
            "Failed to read doofiepack as ZIP for overrides listing: {}",
            e
        );
        AppError::Other(format!(
            "Failed to read doofiepack zip for overrides: {}",
            e
        ))
    })?;
    ...
    info!(
        "Found {} entries in doofiepack. Preparing concurrent streaming for overrides...",
        num_entries
    );
    ...
    for index in 0..num_entries {
        ...
        // Only process the "overrides/" directory for .doofiepack files
        if entry_filename_str.starts_with("overrides/") {
            ...
            let sanitized_relative_path_buf = PathBuf::from(path_after_strip_str)
                ...
                .filter_map(|comp| match comp {
                    ...
                    std::path::Component::ParentDir => {
                        warn!("Parent directory component '..' found and removed in doofiepack override path: {}", path_after_strip_str);
                        None 
                    }
            ...
            if sanitized_relative_path_buf.as_os_str().is_empty() {
                warn!(
                    "Skipping empty sanitized relative path for doofiepack override entry: {} (original relative: {})",
    ...
    if !extraction_tasks.is_empty() {
        ...
        let results = try_join_all(extraction_tasks).await.map_err(|e| {
            error!(
                "Error joining override extraction tasks for doofiepack: {}",
                e
            );
            AppError::Other(format!(
        ...
        info!("Successfully extracted all queued overrides for doofiepack.");
    } else {
        info!("No override files found or queued for extraction in doofiepack.");
    }
...
/// Handles the opening of a .doofiepack file, either on app startup or second instance.
/// It will call the `import_profile` command if a valid file path is found in the arguments.
pub async fn handle_doofiepack_file_paths<R: tauri::Runtime>(
    app_handle: &tauri::AppHandle<R>,
    args: Vec<String>, // Changed from Vec<PathBuf>
) {
    let mut doofiepack_to_import: Option<PathBuf> = None;
    ...
    for arg_str in args_to_check {
        ...
        if let Some(path) = path_candidate {
            if path.is_file() && path.extension().and_then(|ext| ext.to_str()) == Some("doofiepack")
            {
                info!("Found .doofiepack file to process: {}", path.display());
                doofiepack_to_import = Some(path);
                break; // Handle the first .doofiepack file found
            }
        }
    }

    if let Some(file_path_to_import) = doofiepack_to_import {
        if let Some(file_path_str) = file_path_to_import.to_str() {
            ...
            error!(
                "Failed to convert .doofiepack path to string: {}",
                file_path_to_import.display()
            );
        }
    } else {
        info!("No .doofiepack file found in the provided paths.");
    }
}
...
/// Copies a dummy/test `test_doofie_modpacks.json` from the project's source directory
/// (assuming a development environment structure) to the launcher's root directory
/// as `doofie_modpacks.json` if it doesn't already exist.
///
...
pub async fn load_dummy_modpacks() -> Result<()> {
    ...
    let target_file = target_dir.join("doofie_modpacks.json");
    ...
    let source_path = project_root.join("minecraft-data/nrc/doofie_modpacks.json");
    // --- End path resolution ---
