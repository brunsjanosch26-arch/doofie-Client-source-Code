...
const NORISK_API_BASE_URL: &str = "https://api.doofieclient.com/v1";

/// Helper to compute versions file path based on experimental flag
fn doofie_versions_path_for(is_experimental: bool) -> PathBuf {
    let filename = if is_experimental {
        "doofie_versions_exp.json"
    } else {
        "doofie_versions.json"
    };
...
/// Loads standard profiles from the local `doofie_versions.json` file.
...
pub async fn load_local_standard_profiles() -> Result<NoriskVersionsConfig> {
    let file_path = if let Ok(state) = State::get().await {
        ...
        LAUNCHER_DIRECTORY.root_dir().join("doofie_versions.json")
    };
    ...
    let profiles_config: NoriskVersionsConfig = serde_json::from_str(&data).map_err(|e| {
        ...
        AppError::ParseError(format!("Failed to parse doofie_versions.json: {}", e))
    })?;
...
/// Copies a dummy/default `doofie_versions.json` from the project's source directory
...
pub async fn load_dummy_versions() -> Result<()> {
    ...
    let target_file = if let Ok(state) = State::get().await {
        ...
        target_dir.join("doofie_versions.json")
    };
    ...
    let source_path = project_root.join("minecraft-data/nrc/doofie_versions.json");
    // --- End path resolution ---
