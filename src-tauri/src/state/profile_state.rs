...
pub struct Profile {
    ...
    pub selected_doofie_pack_id: Option<String>, // ID of the selected Norisk Pack (e.g., "doofie-prod")
    #[serde(default)] // Keep track of disabled mods per pack/version/loader context
    pub disabled_doofie_mods_detailed: HashSet<NoriskModIdentifier>, // Changed field
    ...
    #[serde(default)]
    pub doofie_information: Option<NoriskInformation>,
...
impl Profile {
    pub async fn effective_doofie_pack_id(&self) -> Option<String> {
        let original = self.selected_doofie_pack_id.as_deref()?;
        Some(
...
impl ProfileManager {
            ...
            match crate::state::state_manager::State::get().await {
                Ok(state) => {
                    if let Some(standard_profile) =
                        state.doofie_version_manager.get_profile_by_id(id).await
                    {
    ...
    pub async fn resolve_and_migrate_pack_id(&self, profile_id: Uuid) -> Result<Option<String>> {
        ...
        profile.selected_doofie_pack_id = Some(fallback.clone());
        self.update_profile(profile_id, profile).await?;
        Ok(Some(fallback))
    }
    ...
    pub async fn set_doofie_mod_status(
        ...
        if let Some(profile) = profiles.get_mut(&profile_id) {
            ...
            let changed;
            if disabled {
                changed = profile.disabled_doofie_mods_detailed.insert(identifier);
            } else {
                changed = profile.disabled_doofie_mods_detailed.remove(&identifier);
            }
    ...
    pub async fn get_profile_instance_path(&self, profile_id: Uuid) -> Result<PathBuf> {
        ...
        match profiles_map.get(&profile_id) {
            ...
            None => {
                ...
                // Get state to access doofie_version_manager
                let state = crate::state::state_manager::State::get().await?;
    ...
    /// Helper function to check if a group belongs to Doofie Client
    fn is_doofie_client_group(group_name: &str) -> bool {
        let normalized = group_name.to_lowercase();
        normalized == "nrc" || normalized == "doofieclient" || normalized == "doofie client"
    }
    ...
    pub fn calculate_group_directory(&self, profile: &Profile) -> Result<PathBuf> {
        if let Some(group) = &profile.group {
            if Self::is_doofie_client_group(group) {
                // Doofie Client groups go to "doofieclient/legacy" for MC < 1.13, "doofieclient/new" otherwise
                if mc_utils::is_legacy_minecraft_version(&profile.game_version) {
                    Ok(default_profile_path().join("doofieclient").join("legacy"))
                } else {
                    Ok(default_profile_path().join("doofieclient").join("new"))
                }
            } else {
    ...
    /// Synchronizes standard profiles by creating editable copies for each doofie_version
    ...
    pub async fn sync_standard_profiles(&self) -> Result<()> {
        ...
        // Get standard profiles from doofie version manager
    ...
    async fn update_copy_with_forced_fields(&self, copy_id: Uuid, standard_profile: &Profile) -> Result<bool> {
        ...
        if let Some(copy) = profiles.get_mut(&copy_id) {
            ...
            // Force update Doofie pack selection if different
            if copy.selected_doofie_pack_id != standard_profile.selected_doofie_pack_id {
                info!("Updating Doofie pack for copy {}: {:?} -> {:?}", copy_id, copy.selected_doofie_pack_id, standard_profile.selected_doofie_pack_id);
                copy.selected_doofie_pack_id = standard_profile.selected_doofie_pack_id.clone();
                changed = true;
            }
...
impl PostInitializationHandler for ProfileManager {
    async fn on_state_ready(&self, _app_handle: Arc<tauri::AppHandle>) -> Result<()> {
        ...
        // Sync standard profiles - create editable copies for each doofie_version
