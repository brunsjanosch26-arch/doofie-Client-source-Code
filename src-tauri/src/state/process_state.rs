...
/// Mod from the launch manifest (incl. disabled). Mirrors backend `CrashModInfo`. `doofie` = pack module.
#[derive(Debug, Clone, Serialize, Deserialize)]
...
/// Snapshot custom + Doofie-pack mods (incl. disabled) for the crash payload.
            ...
            Ok(def) => {
                for entry in &def.mods {
                    ...
                    out.push(CrashModInfo {
                        ...
                        version: None,
                        source: Some("doofie".to_string()),
                        enabled: !profile.disabled_doofie_mods_detailed.contains(&identifier),
                        doofie: true,
                    });
                }
            }
    ...
    pub async fn start_process(
        ...
        > = None;
        {
            let session = crate::utils::log_archive::SessionInfo {
                ...
                doofie_pack: doofie_pack.as_deref(),
