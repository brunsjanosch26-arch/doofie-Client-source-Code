...
const NORISK_VERSIONS_FILENAME: &str = "doofie_versions.json";

/// Returns the path for the doofie versions config depending on experimental mode
pub fn doofie_versions_path_for(is_experimental: bool) -> PathBuf {
    let filename = if is_experimental {
        "doofie_versions_exp.json"
    } else {
...
impl NoriskVersionManager {
    ...
    pub async fn fetch_and_update_config(
        ...
        match DoofieApi::get_standard_versions(doofie_token, is_experimental).await {
            Ok(new_config) => {
                debug!(
    ...
    async fn load_config_internal(&self, path: &PathBuf) -> Result<NoriskVersionsConfig> {
        ...
        match serde_json::from_str(&data) {
            Ok(config) => Ok(config),
            Err(e) => {
                error!("Failed to parse doofie_versions.json at {:?}: {}. Returning default empty config.", path, e);
    ...
    async fn save_config(&self) -> Result<()> {
        ...
        let path_to_write = if let Ok(state) = State::get().await {
            ...
            doofie_versions_path_for(is_exp)
        } else {
        ...
        if let Some(parent_dir) = path_to_write.parent() {
            if !parent_dir.exists() {
                ...
                info!(
                    "Created directory for doofie versions config: {:?}",
                    parent_dir
                );
            }
        }
        ...
        info!(
            "Successfully saved doofie versions config to {:?}",
            path_to_write
        );
        Ok(())
    }
...
/// Returns the default path for the doofie_versions.json file within the launcher directory.
pub fn default_doofie_versions_path() -> PathBuf {
