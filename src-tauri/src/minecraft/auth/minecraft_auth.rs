...
use crate::minecraft::api::DoofieApi;

#[derive(Debug, Serialize, Deserialize)]
pub struct DoofieTokenClaims {
    exp: usize,
    username: String,
}
...
pub struct Credentials {
    ...
    pub doofie_credentials: DoofieCredentials,
    pub active: bool,
...
#[derive(Serialize, Deserialize, Clone, Debug)]
pub struct DoofieCredentials {
    pub production: Option<DoofieToken>,
    pub experimental: Option<DoofieToken>,
}

#[derive(Serialize, Deserialize, Clone, Debug)]
pub struct DoofieToken {
    pub value: String,
    //TODO habs nichts hinbekommen jetzt erstmal bei jedem restart, pub expires: DateTime<Utc>,
}

impl DoofieCredentials {
    ...
    /// Gets the appropriate Doofie token based on the experimental mode setting.
    ///
    /// # Arguments
    ...
    pub fn get_token_for_mode(&self, is_experimental: bool) -> Result<String> {
        ...
        token_option
            ...
            .ok_or_else(|| {
                error!(
                    "No Doofie token found for {} mode.",
                    if is_experimental {
                        "experimental"
                    } else {
                        "production"
...
impl MinecraftAuthStore {
    ...
    pub async fn login_finish_direct_oauth_with_events(&self, code: &str, flow: DirectOAuthFlow, event_id: Uuid) -> Result<Credentials> {
        ...
        let credentials = Credentials {
            ...
            doofie_credentials: match existing_account {
                Some(ref account) => account.doofie_credentials.clone(),
                None => DoofieCredentials {
                    production: None,
                    experimental: None,
                },
            },
    ...
    pub(crate) async fn refresh_doofie_token_if_necessary(
        &self,
        creds: &Credentials,
        force_update: bool,
        ...
        info!(
            "[Token Refresh] Starting Doofie token refresh check for user: {}",
            creds.username
        );
        ...
        if !force_update {
            ...
            let token_ref = if experimental_mode {
                &creds.doofie_credentials.experimental
            } else {
                &creds.doofie_credentials.production
            };
            ...
            if let Some(token) = token_ref {
                ...
                match decode::<DoofieTokenClaims>(&token.value, &key, &validation) {
                    Ok(data) => {
                        info!(
        ...
        if force_update || maybe_update {
            ...
            hasher.update(b"doofie-device-salt");
            hasher.update(&hwid);
            ...
            // Use DoofieApi for token refresh with proper error handling
            info!("[Doofie Token] Starting token refresh using DoofieApi");
            ...
            info!("[Doofie Token] Account is known to have child protection enabled: {}", creds.ignore_child_protection_warning);

            match DoofieApi::refresh_doofie_token_v3(
                &system_id,
                &creds.username,
                &creds.access_token,
            ...
            {
                Ok(doofie_token) => {
                    info!("[Doofie Token] Successfully refreshed token");
                    let mut copied_credentials = creds.clone();

                    if experimental_mode {
                        info!("[Doofie Token] Storing token in experimental credentials");
                        copied_credentials.doofie_credentials.experimental = Some(doofie_token);
                    } else {
                        info!("[Doofie Token] Storing token in production credentials");
                        copied_credentials.doofie_credentials.production = Some(doofie_token);
                    }
                    ...
                    info!("[Doofie Token] Updating account in storage");
    ...
    async fn refresh_token_direct(&self, creds: &Credentials, cred_id: Uuid, profile_name: String) -> Result<Option<Credentials>> {
        ...
        let val = Credentials {
            ...
            refresh_token: oauth_token.value.refresh_token,
            expires: oauth_token.date + Duration::seconds(oauth_token.value.expires_in as i64),
            doofie_credentials: creds.doofie_credentials.clone(),
            active: creds.active,
    ...
    /// 
    /// This method retrieves an account by its ID, refreshes Microsoft and Doofie tokens
    /// if needed, updates the account in storage, and returns the refreshed credentials.
    /// 
    /// # Arguments
    /// * `id` - The UUID of the account to retrieve
    /// * `experimental_mode` - Whether to use experimental mode for Doofie token refresh
    /// 
    /// # Returns
    ...
    pub async fn get_account_by_id_with_refresh(
        ...
        if let Some(creds) = account {
            ...
            let updated_account = self
                .update_doofie_and_microsoft_token(&creds, experimental_mode)
                .await?;
    ...
    pub async fn update_doofie_and_microsoft_token(
        &self,
        creds: &Credentials,
        experimental_mode: bool,
    ) -> Result<Option<Credentials>> {
        ...
        if creds.expires <= Utc::now() + Duration::minutes(5) {
            ...
            match res {
                Ok(val) => {
                    return if val.is_some() {
                        ...
                        Ok(Some(
                            self.refresh_doofie_token_if_necessary(
                                &val.unwrap().clone(),
                                false,
                                experimental_mode,
                            )
            ...
            if creds.ignore_child_protection_warning {
                info!("[Token Check] Skipping Doofie token check due to child protection warning ignore flag");
                Ok(None)
            } else {
                info!("[Token Check] Checking Doofie token status");
                Ok(Some(
                    self.refresh_doofie_token_if_necessary(&creds.clone(), false, experimental_mode)
                        .await?,
                ))
            }
        }
    ...
    pub async fn get_active_account(&self) -> Result<Option<Credentials>> {
        ...
        if let Some(account) = active_account {
            ...
            let updated_account = self
                .update_doofie_and_microsoft_token(&account, is_experimental)
                .await?;
