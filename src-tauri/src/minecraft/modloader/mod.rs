...
impl ModloaderFactory {
    ...
    pub async fn resolve_loader_version(
        ...
        doofie_pack_config: Option<&NoriskModpacksConfig>,
        ...
        if let Some(selected_pack_id) = profile.effective_doofie_pack_id().await {
            if let Some(config) = doofie_pack_config {
                if let Ok(resolved_pack) = config.get_resolved_pack_definition(&selected_pack_id) {
                    if let Some(policy) = &resolved_pack.loader_policy {
                        ...
                        let get_ver = |m: &std::collections::HashMap<String, crate::integrations::doofie_packs::LoaderSpec>| {
