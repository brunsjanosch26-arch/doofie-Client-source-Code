...
impl MinecraftLauncher {
    ...
    fn create_loggable_command_string(command: &std::process::Command) -> String {
        ...
        while let Some(arg_os_str) = args_iter.next() {
            ...
            if arg_str.starts_with("-Ddoofie.token=") {
                parts.push("-Ddoofie.token=*****".to_string());
    ...
    pub async fn launch(
        ...
        let jvm_args = JvmArguments::new(
            natives_path.clone(),
            "doofieclient-launcher".to_string(),
            "3.0.0".to_string(),
            classpath.clone(),
        );
        ...
        //fix for https://github.com/DoofieClient/issues/issues/2357
        ...
        if let Some(p) = &profile {
            command.arg(format!("-Ddoofie.profile.name={}", p.name));
            if let Some(pack_id) = p.selected_doofie_pack_id.as_ref() {
                command.arg(format!("-Ddoofie.pack={}", pack_id));
            }
        }

        // Pass meta dir to game client for shared Discord state file
        command.arg(format!("-Ddoofie.meta.dir={}", crate::config::LAUNCHER_DIRECTORY.meta_dir().display()));
        ...
        let effective_pack = match &profile {
            Some(p) => p.effective_doofie_pack_id().await,
            None => None,
        };
        if effective_pack
            .as_deref()
            ...
            .unwrap_or(false)
        {
            ...
            // Layout matches doofie_assets_download.rs
            // (`<meta>/assets/doofieclient/<bucket>/objects/...`) so any blob
            ...
            // (doofie_modpacks.json). Order is base→priority — client overlays
            // the last entry on top, falling back per-asset.
            let packs_config = state.doofie_pack_manager.get_config().await;
        ...
        let (profile_loader, profile_loader_version, profile_doofie_pack, profile_name, profile_image_url) =
            match profile {
                Some(p) => {
