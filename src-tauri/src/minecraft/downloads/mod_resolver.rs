...
async fn is_filename_blocked_by_config(filename: &str, enable_flagsmith_blocking: bool) -> bool {
    if !enable_flagsmith_blocking {
        return false; // Skip blocking if no Doofie pack is selected
    }
...
pub async fn resolve_target_mods(
    profile: &Profile,
    doofie_config: Option<&NoriskModpacksConfig>,
    ...
    // Enable Flagsmith blocking only if a Doofie pack is selected
    let enable_flagsmith_blocking = profile.selected_doofie_pack_id.is_some();
    
    if enable_flagsmith_blocking {
        debug!("Flagsmith mod blocking is enabled (Doofie pack selected)");
    } else {
        debug!("Flagsmith mod blocking is disabled (no Doofie pack selected)");
    }
    ...
    let effective_pack_id = profile.effective_doofie_pack_id().await;
    if let (Some(pack_id), Some(config)) = (effective_pack_id.as_ref(), doofie_config) {
                ...
                for mod_entry in &pack_definition.mods {
                    ...
                    match ModLoader::from_str(loader_str) {
                        Ok(loader_enum) => {
                            ...
                            if profile.disabled_doofie_mods_detailed.contains(&identifier) {
                                info!(
                    ...
                    if let NoriskModSourceDefinition::Modrinth { project_id, .. } = &mod_entry.source {
                            ...
                            .and_then(|l| l.get(loader_str))
                        {
                            ...
                            {
                                match doofie_packs::get_doofie_pack_mod_filename(
                                    effective_source,
                                    target,
                                    &mod_entry.id,
                                ) {
