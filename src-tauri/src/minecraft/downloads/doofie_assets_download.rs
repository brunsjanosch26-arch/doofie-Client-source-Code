...
const NORISK_ASSETS_DIR: &str = "doofieclient";
...
impl NoriskClientAssetsDownloadService {
    ...
    /// Downloads Doofie client assets for a specific profile, processing the main pack
    ...
    pub async fn download_nrc_assets_for_profile(
        ...
        let main_pack_id = match profile.effective_doofie_pack_id().await {
        ...
        let doofie_token = match token_ref {
            Some(token) => token.value.clone(),
            None => {
                warn!("[NRC Assets Download] No valid Doofie token found for {} mode, skipping asset download",
        ...
        let doofie_packs_config = state.doofie_pack_manager.get_config().await;

        info!(
            "[NRC Assets Download] Resolving pack definition for main pack: {}",
            main_pack_id
        );
        let resolved_pack_definition = doofie_packs_config
        ...
        let target_base_dir = game_directory.join("DoofieClient").join("assets");

        self.emit_progress_event(
            &state,
            profile.id,
            &format!(
                "Starting DoofieClient asset processing for {} groups...",
                total_groups
            ),
            0.01,
            None,
        ...
        // Cleanup wipes anything in `target_base_dir` (= <gameDir>/DoofieClient/assets/)
        ...
        self.emit_progress_event(
            &state,
            profile.id,
            "DoofieClient assets processing completed!",
            1.0,
            None,
        )
        .await?;
    ...
    async fn process_asset_group(
        ...
        let assets =
            match measure_time!(format!("NRC API call '{}'", asset_id), DoofieApi::doofie_assets(asset_id, doofie_token, request_uuid, is_experimental)
                .await)
            {
                Ok(fetched_assets) => {
                    info!(
    ...
    /// Downloads Doofie client assets for a specific asset ID (pack or asset group).
    async fn download_nrc_assets(
        ...
        for (name, asset) in assets_list {
            ...
            let doofie_token_clone = doofie_token.to_string();
            ...
            downloads.push(async move {
                ...
                let url = format!(
                    "{}/{}/assets/{}",
                    "https://cdn.doofie.gg/assets", asset_id_clone, name_clone
                );

                let mut request = HTTP_CLIENT.get(&url);
                request = request.header("Authorization", format!("Bearer {}", doofie_token_clone));
                            ...
                            tokio::spawn({
                                ...
                                async move {
                                    ...
                                    if let Err(e) = state.emit_event(EventPayload {
                                        event_id,
                                        event_type: EventType::DownloadingDoofieClientAssets,
    ...
    async fn emit_copy_event(
        ...
        state
            .emit_event(EventPayload {
                event_id,
                event_type: EventType::CopyingDoofieClientAssets,
                target_id: Some(profile_id),
                message: message.to_string(),
                progress: Some(progress.clamp(0.0, 1.0)),
                error,
