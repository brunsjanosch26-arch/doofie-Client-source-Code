...
pub async fn refresh_permissions() -> Result<(), CommandError> {
    ...
    let token = match active.doofie_credentials.get_token_for_mode(is_experimental) {
        Ok(t) => t,
        Err(e) => {
            warn!(
                "[Permissions] No Doofie token for account {} (experimental={}): {}",
    ...
    match DoofieApi::get_user_permissions(&token, &account_id_str, is_experimental).await {
        Ok(nodes) => {
