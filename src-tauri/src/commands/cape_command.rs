...
pub async fn browse_capes(
    ...
    // Get the Doofie token: prioritize passed token, otherwise get from active account
    let token_to_use = match payload.doofie_token {
        Some(token) => {
            debug!("Using provided Doofie token.");
            token
        }
        None => {
...
pub struct GetPlayerCapesPayload {
    ...
    pub doofie_token: Option<String>,
...
/// - doofie_token: Optional Doofie token
#[tauri::command]
pub async fn get_player_capes(
    ...
    let token_to_use = match payload.doofie_token {
        Some(token) => {
            debug!("[CMD get_player_capes] Using doofie_token from payload.");
            token
        }
        None => {
            debug!("[CMD get_player_capes] No doofie_token in payload, attempting to use token from active account.");
            let acc = active_account_opt.as_ref().ok_or_else(|| {
                error!("[CMD get_player_capes] Doofie token required (neither in payload nor from active account).");
                CommandError::from(AppError::NoCredentialsError)
            })?;
            acc.doofie_credentials.get_token_for_mode(is_experimental)?
        }
    };
    debug!(
...
pub async fn get_owned_capes_list(
    ...
    let token_to_use = match doofie_token {
        Some(token) => token,
        None => active_account
            .doofie_credentials
            .get_token_for_mode(is_experimental)?,
...
pub async fn download_template_and_open_explorer(
    ...
    let template_url = if is_experimental {
        format!("https://cdn.doofie.gg/capes-staging/{}", template_file)
    } else {
        format!("https://cdn.doofie.gg/capes/{}", template_file)
    };
