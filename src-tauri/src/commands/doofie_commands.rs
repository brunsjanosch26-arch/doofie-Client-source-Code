...
use crate::minecraft::api::doofie_api::{AdventCalendarDay, CrashlogDto, DoofieApi, ReferralInfo, Reward, UniquePlayersResponse, UserNotification};
...
pub async fn discord_auth_link(app: AppHandle) -> Result<(), CommandError> {
    ...
    let doofie_creds = &selected_account_arc.doofie_credentials;
    let token = doofie_creds.get_token_for_mode(is_experimental)?;

    let url_string = format!(
        "https://api{}.doofie.gg/api/v1/core/oauth/discord?token={}",
    ...
    while (Utc::now() - start_time) < ChronoDuration::minutes(10) {
        match window.url().map_err(|e| {
            ...
            Ok(current_url) => {
                ...
                if current_url_str
                    .starts_with("https://api.doofie.gg/api/v1/core/oauth/discord/complete")
                    || current_url_str.starts_with(
                        "https://api-staging.doofie.gg/api/v1/core/oauth/discord/complete",
                    )
                {
...
pub async fn discord_auth_status() -> Result<bool, CommandError> {
    ...
    Ok(DoofieApi::discord_link_status(&token, &account_id_str, is_experimental).await?)
}
...
pub async fn discord_auth_unlink() -> Result<(), CommandError> {
    ...
    DoofieApi::unlink_discord(&token, &account_id_str, is_experimental).await?;
    Ok(())
}
...
pub async fn github_auth_link(app: AppHandle) -> Result<(), CommandError> {
    ...
    let url_string = format!(
        "https://api{}.doofie.gg/api/v1/core/oauth/github?token={}",
        if is_experimental { "-staging" } else { "" },
        token
    );
    ...
    while (Utc::now() - start_time) < ChronoDuration::minutes(10) {
        match window.url().map_err(|e| {
            ...
            Ok(current_url) => {
                let current_url_str = current_url.as_str();
                if current_url_str
                    .starts_with("https://api.doofie.gg/api/v1/core/oauth/github/complete")
                    || current_url_str.starts_with(
                        "https://api-staging.doofie.gg/api/v1/core/oauth/github/complete",
                    )
                {
...
pub async fn github_auth_status() -> Result<bool, CommandError> {
    ...
    Ok(DoofieApi::github_link_status(&token, &account_id_str, is_experimental).await?)
}
...
pub async fn github_auth_unlink() -> Result<(), CommandError> {
    ...
    DoofieApi::unlink_github(&token, &account_id_str, is_experimental).await?;
    Ok(())
}
...
pub async fn check_crash_log_command(
    ...
    let result = DoofieApi::check_crash_log(
        &token,
        &payload,
...
pub async fn get_mobile_app_token() -> Result<String, CommandError> {
    ...
    Ok(DoofieApi::get_mcreal_app_token(&token, &account_id_str, is_experimental).await?)
}
...
pub async fn reset_mobile_app_token() -> Result<String, CommandError> {
    ...
    Ok(DoofieApi::reset_mcreal_app_token(&token, &account_id_str, is_experimental).await?)
}
...
pub async fn get_advent_calendar_command() -> Result<Vec<AdventCalendarDay>, CommandError> {
    ...
    Ok(DoofieApi::get_advent_calendar(&token, &account_id_str, is_experimental).await?)
}
...
pub async fn claim_advent_calendar_day_command(tag: u32) -> Result<AdventCalendarDay, CommandError> {
    ...
    Ok(DoofieApi::claim_advent_calendar_day(&token, tag, &account_id_str, is_experimental).await?)
}
...
pub async fn get_referral_info(code: String) -> Result<ReferralInfo, CommandError> {
    ...
    Ok(DoofieApi::get_referral_info(&code, is_experimental).await?)
}
...
pub async fn get_notifications() -> Result<Vec<UserNotification>, CommandError> {
    ...
    Ok(DoofieApi::get_notifications(&token, &account_id_str, is_experimental).await?)
}
...
pub async fn mark_all_notifications_read() -> Result<(), CommandError> {
    ...
    DoofieApi::mark_all_notifications_read(&token, &account_id_str, is_experimental).await?;
    Ok(())
}
...
pub async fn mark_notification_read(notification_id: String) -> Result<(), CommandError> {
    ...
    DoofieApi::mark_notification_read(&token, &notification_id, &account_id_str, is_experimental).await?;
    Ok(())
}
...
pub async fn get_unique_players_24h_command() -> Result<UniquePlayersResponse, CommandError> {
    ...
    let state = State::get().await?;
    let is_experimental = state.config_manager.is_experimental_mode().await;
    Ok(DoofieApi::get_unique_players_24h(is_experimental).await?)
}