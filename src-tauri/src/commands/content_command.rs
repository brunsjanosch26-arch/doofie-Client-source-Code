            ...
            Ok(profile_instance_path) => {
                ...
                if let Some(ref ct) = content_type {
                    dirs_to_scan = match ct {
                        ...
                        _ => dirs_to_scan, // DoofieMod or others: scan all
                    };
                }
...
pub struct ToggleContentPayload {
    ...
    file_path: Option<String>,
    enabled: bool,
    doofie_mod_identifier: Option<crate::state::profile_state::NoriskModIdentifier>,
...
pub async fn toggle_content_from_profile(
    ...
    log::info!(
        "Attempting to toggle content state: profile_id={}, sha1_hash={:?}, file_path={:?}, enabled={}, doofie_mod_identifier={:?}, content_type={:?}",
        payload.profile_id,
        payload.sha1_hash,
        payload.file_path,
        payload.enabled,
        payload.doofie_mod_identifier,
    ...
    // Handle Doofie Pack item toggling if the identifier is provided
    if let Some(doofie_mod_identifier) = payload.doofie_mod_identifier {
        log::info!(
            "Toggling Doofie Pack item state: profile={}, pack={}, mod={}, disabled={}",
            payload.profile_id,
            doofie_mod_identifier.pack_id,
            doofie_mod_identifier.mod_id,
            !payload.enabled
        );

        // Clone the fields needed for logging
        let pack_id = doofie_mod_identifier.pack_id.clone();
        let mod_id = doofie_mod_identifier.mod_id.clone();

        // Call set_doofie_mod_status with the appropriate parameters
        match state_manager
            .profile_manager
            .set_doofie_mod_status(
                ...
                doofie_mod_identifier.pack_id,
                doofie_mod_identifier.mod_id,
                doofie_mod_identifier.game_version,
                doofie_mod_identifier.loader,
            ...
            .await
        {
            Ok(_) => {
                log::info!(
                    "Successfully toggled Doofie Pack item state for pack_id={}, mod_id={} to enabled={}",
                    pack_id,
                    mod_id,
                    payload.enabled
                );
            ...
            Err(e) => {
                log::error!("Failed to toggle Doofie Pack item state: {}", e);
                return Err(CommandError::from(e));
            }
        }
    }

    // Continue with SHA1-based content toggling if not a Doofie Pack item
    let current_sha1_hash = match payload.sha1_hash {
        Some(ref hash) => hash.clone(),
        None => {
            log::warn!("SHA1 hash is required for the current toggle implementation when not toggling a Doofie Pack item.");
            return Err(CommandError::from(AppError::Other(
                "SHA1 hash is required for this toggle operation when not toggling a Doofie Pack item.".to_string(),
            )));
        }
    };
    ...
    match payload.content_type {
        ...
        Some(profile_utils::ContentType::DoofieMod) => {
            log::debug!(
                "Targeted toggle for DoofieMod with SHA1: {}",
                current_sha1_hash
            );
            // DoofieMods are handled differently, not by scanning directories
            // We don't need to scan any asset types for DoofieMods
...
pub async fn install_content_to_profile(
    ...
    match payload.content_type {
        ...
        profile_utils::ContentType::DoofieMod => {
            log::info!("DoofieMod installation is not supported via this unified command");
            Err(CommandError::from(AppError::Other(
                "DoofieMod installation not supported via this command".to_string(),
            )))
        }
...
pub async fn install_local_content_to_profile(
    ...
    match payload.content_type {
        ...
        profile_utils::ContentType::DoofieMod => {
            log::error!(
                "ContentType::DoofieMod is not supported for local installation via this command. Profile: {}",
                payload.profile_id
            );
            return Err(CommandError::from(AppError::Other(
                "Local installation of DoofieMod content type is not supported.".to_string(),
            )));
        }
...
pub async fn switch_content_version(
    ...
    match payload.content_type {
        ...
        profile_utils::ContentType::DoofieMod => {
            log::error!("Switching version for DoofieMod is not supported via this command.");
            Err(CommandError::from(AppError::InvalidOperation(
                "DoofieMod versions are managed by pack configuration.".to_string(),
            )))
        }
    }
}
