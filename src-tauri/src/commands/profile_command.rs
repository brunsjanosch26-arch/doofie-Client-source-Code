...
pub struct CreateProfileParams {
    ...
    selected_doofie_pack_id: Option<String>,
...
pub struct UpdateProfileParams {
    ...
    use_shared_minecraft_folder: Option<bool>,
    clear_selected_doofie_pack: Option<bool>,
    doofie_information: Option<crate::state::profile_state::NoriskInformation>,
...
pub async fn create_profile(params: CreateProfileParams) -> Result<Uuid, CommandError> {
    ...
    let profile = Profile {
        ...
        mods: Vec::new(),
        selected_doofie_pack_id: params.selected_doofie_pack_id.clone(),
        disabled_doofie_mods_detailed: HashSet::new(),
...
pub async fn launch_profile(
    ...
    let profile = match state.profile_manager.get_profile(id).await {
        ...
        Err(_) => {
            ...
            let standard_versions = state.doofie_version_manager.get_config().await;
        ...
        if let Ok(state) = State::get().await {
            ...
            match install_result {
                Ok(_) => {
                    ...
                    if let Some(pack_id) = profile_clone.selected_doofie_pack_id.clone() {
                        ...
                        track_analytics("play_doofie_pack", pack_props);
                    }
...
async fn try_update_profile(id: Uuid, params: UpdateProfileParams) -> Result<(), CommandError> {
    ...
    // Handle selected_doofie_pack_id based on clear_selected_doofie_pack and new value
    if params.clear_selected_doofie_pack == Some(true) {
        info!("Clearing selected_doofie_pack_id for profile {}", id);
        profile.selected_doofie_pack_id = None;
    } else if let Some(pack_id) = &params.selected_doofie_pack_id {
        info!(
            "Updating selected_doofie_pack_id to: {} for profile {}",
            pack_id, id
        );
        profile.selected_doofie_pack_id = Some(pack_id.clone());
    } else {
        info!("selected_doofie_pack_id not explicitly changed or cleared for profile {}. Current: {:?}", id, profile.selected_doofie_pack_id);
        // No change to selected_doofie_pack_id if neither clear is true nor a new value is provided
    }
    ...
    // Handle doofie_information
    if let Some(doofie_info) = params.doofie_information {
        info!("Updating doofie_information to: {:?}", doofie_info);
        profile.doofie_information = Some(doofie_info);
    } else {
        // This else block handles the case where `doofie_information` is explicitly `null` in JSON,
        ...
        // If you want `null` to clear it, you would do: `profile.doofie_information = None;`
        info!(
            "doofie_information not provided or explicitly null, keeping existing: {:?}",
            profile.doofie_information
        );
    }
...
pub async fn resolve_loader_version(
    ...
    let doofie_pack_config = state.doofie_pack_manager.get_config().await;

    let resolved = ModloaderFactory::resolve_loader_version(
        &profile,
        &minecraft_version,
        Some(&doofie_pack_config),
    ).await;
...
pub async fn get_standard_profiles() -> Result<NoriskVersionsConfig, CommandError> {
    ...
    let state = State::get().await?;
    let config = state.doofie_version_manager.get_config().await;
    Ok(config)
}
...
#[tauri::command]
pub async fn get_doofie_packs() -> Result<NoriskModpacksConfig, CommandError> {
    info!("Received command get_doofie_packs");
    let state = State::get().await?;
...
#[tauri::command]
pub async fn get_doofie_packs_resolved() -> Result<NoriskModpacksConfig, CommandError> {
    info!("Received command get_doofie_packs_resolved");
    let state = State::get().await?;
    let manager = &state.doofie_pack_manager; // Get a reference
...
#[tauri::command]
pub async fn set_doofie_mod_status(
    profile_id: Uuid,
    pack_id: String,
    mod_id: String,
    game_version: String,
    ...
    info!(
        "Received command set_doofie_mod_status: profile={}, pack={}, mod={}, mc={}, loader={}, disabled={}",
    ...
    state
        .profile_manager
        .set_doofie_mod_status(profile_id, pack_id, mod_id, game_version, loader, disabled)
        .await?;
    Ok(())
}
...
pub async fn import_profile_from_file(app_handle: tauri::AppHandle) -> Result<(), CommandError> {
    ...
    let dialog_result = tokio::task::spawn_blocking(move || {
        app_handle
            .dialog()
            .file()
            .add_filter("Modpack Files", &["mrpack", "doofiepack", "zip"])
            .set_title("Select Modpack File (.mrpack, .doofiepack, or .zip)")
    ...
    if let Some(file_path_obj) = dialog_result {
        ...
        let new_profile_id = match file_extension.as_deref() {
            ...
            Some("doofiepack") => {
                log::info!("File extension is .doofiepack, proceeding with doofiepack processing.");
                crate::integrations::doofie_packs::import_doofiepack_as_profile(file_path_buf, None)
                    .await?
            }
            Some("zip") => {
            ...
            _ => {
                ...
                return Err(CommandError::from(AppError::Other(
                    "Invalid file type selected. Please select a .mrpack, .doofiepack, or .zip file."
                        .to_string(),
                )));
            }
        };
...
pub async fn import_profile(file_path_str: String, event_id: Option<String>) -> Result<Uuid, CommandError> {
    ...
    let new_profile_id = match file_extension.as_deref() {
        ...
        Some("doofiepack") => {
            log::info!("File extension is .doofiepack, proceeding with doofiepack processing.");
            crate::integrations::doofie_packs::import_doofiepack_as_profile(file_path_buf, event_id_uuid).await?
        }
        Some("zip") => {
            log::info!("File extension is .zip, proceeding with CurseForge modpack processing.");
...
pub async fn copy_profile(params: CopyProfileParams) -> Result<Uuid, CommandError> {
    ...
    let new_profile = Profile {
        ...
        selected_doofie_pack_id: source_profile.selected_doofie_pack_id.clone(),
        disabled_doofie_mods_detailed: source_profile.disabled_doofie_mods_detailed.clone(),
        ...
        doofie_information: source_profile.doofie_information.clone(),
...
/// Exports a profile to a .doofiepack file format with a fixed export directory
#[tauri::command]
pub async fn export_profile(
    ...
    // Sanitize the filename and add .doofiepack extension
    ...
    let doofiepack_filename = format!("{}.doofiepack", sanitized_name);

    // Create full export path
    let export_path = exports_dir.join(&doofiepack_filename);

    info!("Exporting profile to {}", export_path.display());

    // Perform the export
    let result_path = profile_utils::export_profile_to_doofiepack(
        params.profile_id,
...
#[tauri::command]
pub async fn refresh_doofie_packs() -> Result<(), CommandError> {
...
pub async fn refresh_standard_versions() -> Result<Vec<Profile>, CommandError> {
        ...
        .await
    {
        Ok(_) => {
            ...
            let standard_profiles = state.doofie_version_manager.get_config().await.profiles;
...
fn apply_overrides(profile: &mut Profile, ov: LaunchOverrides) -> Result<(), CommandError> {
    ...
    if let Some(p) = ov.pack {
        profile.selected_doofie_pack_id = Some(p);
    }
    Ok(())
}
...
/// at `<root>/profiles/doofieclient/temp/<uuid>/` and is swept into the trash
...
pub async fn launch_temp_profile(args: TempLaunchArgs) -> Result<(), CommandError> {
    ...
    let profile = Profile {
        ...
        state: ProfileState::NotInstalled,
        mods: Vec::new(),
        selected_doofie_pack_id: args.pack.clone(),
        disabled_doofie_mods_detailed: HashSet::new(),
        source_standard_profile_id: None,
