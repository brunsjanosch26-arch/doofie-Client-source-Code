...
async fn get_auth_info() -> Result<(String, Uuid, String, bool), CommandError> {
    ...
    let doofie_token = if is_experimental {
        account.doofie_credentials.experimental.as_ref()
    } else {
        account.doofie_credentials.production.as_ref()
    }
    .ok_or_else(|| CommandError { message: "No Doofie token available".to_string(), kind: "NoToken".to_string(), ..Default::default() })?;

    Ok((
        doofie_token.value.clone(),
        account.id,
