...
use crate::integrations::doofie_versions;
...
use commands::profile_command::{
    ...
    get_doofie_packs, get_doofie_packs_resolved, get_profile, get_profile_directory_structure,
    ...
    open_profile_latest_log, refresh_doofie_packs, refresh_standard_versions, repair_profile,
    resolve_loader_version, search_profiles, set_custom_mod_enabled, set_doofie_mod_status,
...
use commands::file_command::{
    delete_file, get_icons_for_archives, get_icons_for_doofie_mods, list_all_mc_logs,
...
// Import NRC commands
use commands::doofie_commands::{check_update_available_command, download_and_install_update_command, get_news_and_changelogs_command, get_advent_calendar_command, claim_advent_calendar_day_command, get_unique_players_24h_command};
...
async fn main() {
    ...
    tauri::Builder::default()
        ...
        .setup(|app| {
            ...
            let show_item = MenuItem::with_id(app, "show", "Show Doofie Launcher", true, None::<&str>)?;
            ...
            // --- Handle .doofiepack file opening on initial startup (all platforms) ---
            ...
            if startup_args.len() > 1 { // args[0] is exe path, check if there are more
                ...
                tauri::async_runtime::spawn(async move {
                    // Pass all startup_args; handle_doofiepack_file_paths will skip the exe path if needed
                    doofie_packs::handle_doofiepack_file_paths(&handle_clone, startup_args).await;
                });
            }*/
                        ...
                        Err(e) => {
                            ...
                            #[cfg(target_os = "windows")]
                            {
                                ...
                                let _ = state_init_app_handle
                                    .dialog()
                                    .message("The Doofie Launcher encountered a critical error.\n\n\
                                        Please join our Discord for support:\n\
                                        https://discord.doofie.gg")
                                    .kind(MessageDialogKind::Error)
                                    .title("Doofie Launcher - Critical Error")
                                    .blocking_show();
                            }
        ...
        .invoke_handler(tauri::generate_handler![
            ...
            get_doofie_packs,
            get_doofie_packs_resolved,
            set_doofie_mod_status,
            ...
            commands::doofie_commands::discord_auth_link,
            commands::doofie_commands::discord_auth_status,
            commands::doofie_commands::discord_auth_unlink,
            commands::doofie_commands::github_auth_link,
            commands::doofie_commands::github_auth_status,
            commands::doofie_commands::github_auth_unlink,
            commands::doofie_commands::check_crash_log_command,
            commands::crash_fix_command::apply_crash_fix,
            commands::crash_fix_command::revert_crash_fix,
            commands::doofie_commands::log_message_command,
            ...
            commands::doofie_commands::get_mobile_app_token,
            commands::doofie_commands::reset_mobile_app_token,
            commands::doofie_commands::get_advent_calendar_command,
            commands::doofie_commands::claim_advent_calendar_day_command,
            commands::doofie_commands::get_unique_players_24h_command,
            commands::doofie_commands::get_referral_info,
            commands::doofie_commands::get_notifications,
            commands::doofie_commands::mark_all_notifications_read,
            commands::doofie_commands::mark_notification_read,
            get_capes_by_hashes,
